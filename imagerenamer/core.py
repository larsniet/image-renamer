...
def rename_images(folder_path, create_backup=False, format_string="%Y-%m-%d_%H-%M-%S", callback=None):
    """
    Rename all image files in the folder based on their creation date.
    ...
    if not os.path.isdir(folder_path):
        ...
        if callback:
            ...
            print(message)
        return {"total": 0, "renamed": 0, "skipped": 0, "error": True}
    ...
    for file in os.listdir(folder_path):
        ...
        if os.path.isdir(file_path) or not file.lower().endswith(image_extensions):
            continue
        image_files.append(file)
    
    total_files = len(image_files)
    message = f"Found {total_files} image files to process"
    if callback:
        callback(message)
    else:
        print(message)
